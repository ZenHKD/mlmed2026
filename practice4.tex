...
\textbf{Validation.} At the end of each epoch, the model is evaluated on the held-out validation subset. I compute per-voxel Dice coefficient and Intersection-over-Union (IoU) for the lung region. The best checkpoint is selected based on the lowest validation loss. Figure~\ref{fig:stage1_curves} shows the training curves.
...
\end{itemize}
The best checkpoint is selected based on the highest validation AUC-ROC, as it is threshold-independent and more robust for imbalanced datasets than loss-based selection. Figures~\ref{fig:stage2_loss}--\ref{fig:stage2_metrics} show the training curves.
